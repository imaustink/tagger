(function($) {
    ...
    $.fn.extend({
        tagger: function(options) {
            ...
            if(typeof options.styles === 'object' && Array.isArray(options.styles)){
                for(var styles in options.styles){
                    if(options.styles.hasOwnProperty(styles)){
                        styles_obj[styles] = options.styles[styles];
                    }
                }
            }
            ...
            this.add = function(value){
                ...
                var tag = $('<span>', {class: classes(options.color)}).css(styles_obj['.tagger-label']).append(
                    $('<span>', {class: 'tagger-content', 'contenteditable': true}).text(value).blur(function(){
                        ...
                        // Remove tag
                        self.remove(tag);
                        // Add tag back if it has text and isn't a dup
                        if(text && !self.exists(text)) self.add(text);
                    ...
                    '&nbsp;',
                    $('<a>', {class: 'glyphicon glyphicon-remove'}).css(styles_obj['.tagger-remove-tag']).click(function(){
                        self.remove(tag);
                    })
                );
                // Add drag handle
                if(options.sortable) tag.prepend(
                    $('<span>', {class: 'glyphicon glyphicon-menu-hamburger mr5 tagger-handle'}).css(styles_obj['.tagger-handle'])
                );
                // Add tag
            ...
            // Set input classes
            $input.css(styles_obj['.tagger-input']);

            // Set input type to text
            $input.attr('type', 'text');
            
            // Set container styles
            $container.css({'height': 'auto', 'line-height': '27px'});
            ...
            if(options.sortable){
                Sortable.create($container[0], {
                    ...
                    onEnd: function(){
                        ...
                        $input.show();
                        // Trigger our event
                        triggerMany($input, 'tagger:change tagger:sort', [self.getValues()]);
                    }
                });
            }
